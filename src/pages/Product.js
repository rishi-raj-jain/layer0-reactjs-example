      ...
      <div className="flex w-full flex-col items-start md:flex-row">
        <div className="relative flex w-full flex-col items-start md:w-[65%]">
          <div className="absolute top-0 left-0 flex flex-col items-start">
            {data.name && <h3 className="bg-white py-2 px-4 text-2xl font-bold text-black">{data.name}</h3>}
            {data.prices && (
        ...
        <div className="flex w-full flex-col items-start px-10 md:w-[35%]">
          {data.name && <h1 className="mt-10 text-3xl font-bold text-white md:mt-0">{data.name}</h1>}
          {data.description && <h2 dangerouslySetInnerHTML={{ __html: data.description }} className="text-md mt-5 font-light text-[#FFFFFF75]"></h2>}
        ...
        <div className="relative mt-10 flex w-full flex-col">
          ...
          <div className="product-thumbnails flex flex-row items-start overflow-x-scroll">
            {relatedProducts.map((i) => (
              <Link key={i.images[0].url} to={`/product${i.path}`}>
                <img
                  loading="lazy"
                  key={i.images[0].url}
